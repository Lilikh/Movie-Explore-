...
const WatchList = () => {
  ...
  const handleFavoriteToggle = (movie, isFavorite) => {
    if (!isFavorite) {
      ...
      <div className="flex flex-wrap justify-center">
        {favoriteMovies.length > 0 ? (
          favoriteMovies.map((movie) => (
            <div
              key={movie.imdbID}
              className="border border-gray-300 bg-green-950 w-[200px] m-2 rounded-md text-center text-white cursor-pointer transition-transform duration-300 transform hover:scale-110"
            ...
            >
              ...
              <div className="flex justify-center gap-3 text-base p-2">
                <h3>{movie.Title}</h3>
                <FavoriteButton movie={movie} onFavoriteToggle={handleFavoriteToggle} />
              </div>
            </div>
          ))
        ) : (
